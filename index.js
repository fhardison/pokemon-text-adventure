...
const loadDisk = (disk, config = {}) => {
  ...
  const defaults = {
    // retrieve user input
    getInput: () => document.querySelector('#input').value.trim(),
        ...
        const cmds = {
          ...
          go() {
            ...
            if (!nextRoom) {
              ...
              const nr = getRoom(nextRoom.id);
              println(nr.restriction);
              if (nr.restriction){
                if (nr.canEnter({disk, println, getRoom, enterRoom})){
                  enterRoom(nextRoom.id);
                } else {
                  println('Sorry, you can\'t enter here right now')
                }
              } else {
                enterRoom(nextRoom.id);
              }
